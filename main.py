...
def ntool():
    ...
    admin = Panel(
        Align.center(Text.from_ansi(
            f"{C.INFO}(1): {C.WHITE}Key Free{C.RESET}\n"
            f"{C.INFO}(2): {C.WHITE}Key Pro{C.RESET}"
        )),
        title="[bold cyan]Menu Key[/bold cyan]",
        title_align="center",
        width=box_width,
        border_style="yellow",
    )
    ...
    console.print(Align.center(admin))
    print("")
    key = input(f"{C.INFO}[~] {C.WHITE}Nhập lựa chọn của bạn: {C.RESET}")
    if key == "1":
        console.print(f"[cyan]You have selected Key Free![/cyan]")
    elif key == "2":
        console.print(f"[cyan]You have selected Key Pro![/cyan]")
        api_key = input(f"{C.INFO}[~] {C.WHITE}Enter your API key: {C.RESET}")
    else:
        console.print(f"[red]Invalid choice. Please select 1 or 2.[/red]")
